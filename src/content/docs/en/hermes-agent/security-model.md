---
title: "Hermes Agent: Security Model"
description: "Security Model for Hermes Agent: source-linked guidance, task maps, tradeoffs, pitfalls, and update notes."
---
...
- Search intent: Understand which Hermes protections are real security boundaries.
- Core question: Which mechanisms are containment, which are heuristics, and what should be isolated at the OS level?
- Differentiated angle: Emphasize the official trust model: approval gates and scanners help, but adversarial LLM containment needs OS-level isolation.
- Research baseline: 2026-05-22/23

## Editorial Notes

- The official security policy is unusually explicit: for an adversarial LLM, the load-bearing boundary is OS-level isolation. Approval gates, redaction, scanners, and allowlists are useful heuristics, not containment.
- This page should be read before exposing Hermes to untrusted web content, inbound email, public messaging channels, third-party skills, plugins, or MCP servers.
...
Recheck the official documentation, package metadata, repository README, release or changelog notes, and any linked API reference before updating claims about Security Model. Which mechanisms are containment, which are heuristics, and what should be isolated at the OS level?
