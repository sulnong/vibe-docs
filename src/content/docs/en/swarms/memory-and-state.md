---
title: "Swarms: Memory and State"
description: "Memory and State for Swarms: source-linked guidance, task maps, tradeoffs, pitfalls, and update notes."
---

# Swarms: Memory and State
...
- Search intent: Use Swarms persistent memory, conversation archives, and context compression without state leaks.
- Core question: How does agent identity affect persisted history, and when should context be compacted or disabled?
- Differentiated angle: Highlight that `agent_name` is a state key; accidental name reuse can contaminate later runs.
- Research baseline: 2026-05-22/23

## Editorial Notes

- Persistent memory is keyed by `agent_name`. Reusing names intentionally resumes state; reusing names accidentally can pollute future runs.
- Swarms v12 made disk-backed memory and context compression a central behavior change.
...
Recheck the official documentation, package metadata, repository README, release or changelog notes, and any linked API reference before updating claims about Memory and State. How does agent identity affect persisted history, and when should context be compacted or disabled?
