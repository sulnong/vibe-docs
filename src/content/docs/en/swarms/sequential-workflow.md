...
description: "SequentialWorkflow for Swarms: source-linked guidance, task maps, tradeoffs, pitfalls, and update notes."
---
...
- Search intent: Use SequentialWorkflow when each agent depends on the previous output.
- Core question: How should the data contract between steps be written, verified, and guarded against drift?
- Differentiated angle: Make the handoff contract explicit: one agent output becomes the next agent input.
...
Recheck the official documentation, package metadata, repository README, release or changelog notes, and any linked API reference before updating claims about SequentialWorkflow. How should the data contract between steps be written, verified, and guarded against drift?
