---
title: "Postgres Best Practices Skill"
description: "Postgres Best Practices Skill: Applies PostgreSQL best practices for Supabase-backed projects."
---

# Postgres Best Practices Skill

Applies PostgreSQL best practices for Supabase-backed projects.

This page helps you decide whether to install or invoke `supabase/postgres-best-practices`, and how to keep the first use bounded.
...
| Directory listing | https://officialskills.sh/supabase/skills/postgres-best-practices |
| Primary source | https://officialskills.sh/supabase/skills/postgres-best-practices |
...
Start from the listing page: https://officialskills.sh/supabase/skills/postgres-best-practices

If the page provides an install command, copy the command from the listing instead of reconstructing it from the URL. If your agent client supports GitHub skill paths, use the primary source: https://officialskills.sh/supabase/skills/postgres-best-practices
...
- Whether `supabase/postgres-best-practices` is better than a one-off prompt for this task.
...
- If the task is closer to "Designs efficient MongoDB document schemas with validation and indexing patterns.", check [MongoDB Schema Design Skill](/en/awesome-skills/mongodb-schema-design/) first; use this page when the focus remains "Applies PostgreSQL best practices for Supabase-backed projects.".
- If the task is closer to "Analyzes and optimizes MongoDB query performance.", check [MongoDB Query Optimizer Skill](/en/awesome-skills/mongodb-query-optimizer/) first; use this page when the focus remains "Applies PostgreSQL best practices for Supabase-backed projects.".
- If the task is closer to "Guides Redis development across data structures, caching, vector search, and performance.", check [Redis Development Skill](/en/awesome-skills/redis-development/) first; use this page when the focus remains "Applies PostgreSQL best practices for Supabase-backed projects.".
- If the task is closer to "Applies best practices for working with ClickHouse.", check [ClickHouse Best Practices Skill](/en/awesome-skills/clickhouse-best-practices/) first; use this page when the focus remains "Applies PostgreSQL best practices for Supabase-backed projects.".
