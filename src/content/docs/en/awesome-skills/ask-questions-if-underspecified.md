---
title: "Ask Questions If Underspecified Skill"
description: "Ask Questions If Underspecified Skill: Forces clarification when requirements are too ambiguous for safe security-sensitive work."
---

# Ask Questions If Underspecified Skill

Forces clarification when requirements are too ambiguous for safe security-sensitive work.

This page helps you decide whether to install or invoke `trailofbits/ask-questions-if-underspecified`, and how to keep the first use bounded.

## Quick facts

| Field | Value |
| --- | --- |
| Skill | `trailofbits/ask-questions-if-underspecified` |
| Category | [Security and Review](/en/awesome-skills/categories/security-review/) |
| Directory listing | https://officialskills.sh/trailofbits/skills/ask-questions-if-underspecified |
| Primary source | https://officialskills.sh/trailofbits/skills/ask-questions-if-underspecified |
...
Start from the listing page: https://officialskills.sh/trailofbits/skills/ask-questions-if-underspecified

If the page provides an install command, copy the command from the listing instead of reconstructing it from the URL. If your agent client supports GitHub skill paths, use the primary source: https://officialskills.sh/trailofbits/skills/ask-questions-if-underspecified
...
- Whether `trailofbits/ask-questions-if-underspecified` is better than a one-off prompt for this task.
...
- If the task is closer to "Reviews code for language-specific security vulnerabilities and practical hardening opportunities.", check [Security Best Practices Skill](/en/awesome-skills/security-best-practices/) first; use this page when the focus remains "Forces clarification when requirements are too ambiguous for safe security-sensitive work.".
- If the task is closer to "Creates repository-specific threat models by identifying assets, trust boundaries, and attack paths.", check [Security Threat Model Skill](/en/awesome-skills/security-threat-model/) first; use this page when the focus remains "Forces clarification when requirements are too ambiguous for safe security-sensitive work.".
- If the task is closer to "Builds deep architectural context before security audit or investigation work.", check [Audit Context Building Skill](/en/awesome-skills/audit-context-building/) first; use this page when the focus remains "Forces clarification when requirements are too ambiguous for safe security-sensitive work.".
- If the task is closer to "Uses static analysis tools such as CodeQL, Semgrep, and SARIF workflows.", check [Static Analysis Skill](/en/awesome-skills/static-analysis/) first; use this page when the focus remains "Forces clarification when requirements are too ambiguous for safe security-sensitive work.".
...
- [Static Analysis Skill](/en/awesome-skills/static-analysis/) - Uses static analysis tools such as CodeQL, Semgrep, and SARIF workflows.
