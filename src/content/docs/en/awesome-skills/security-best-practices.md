...
description: "Security Best Practices Skill: Reviews code for language-specific security vulnerabilities and practical hardening opportunities."
---

# Security Best Practices Skill

Reviews code for language-specific security vulnerabilities and practical hardening opportunities.

This page helps you decide whether to install or invoke `openai/security-best-practices`, and how to keep the first use bounded.
...
| Directory listing | https://officialskills.sh/openai/skills/security-best-practices |
| Primary source | https://officialskills.sh/openai/skills/security-best-practices |
...
Start from the listing page: https://officialskills.sh/openai/skills/security-best-practices

If the page provides an install command, copy the command from the listing instead of reconstructing it from the URL. If your agent client supports GitHub skill paths, use the primary source: https://officialskills.sh/openai/skills/security-best-practices
...
- Whether `openai/security-best-practices` is better than a one-off prompt for this task.
...
- If the task is closer to "Creates repository-specific threat models by identifying assets, trust boundaries, and attack paths.", check [Security Threat Model Skill](/en/awesome-skills/security-threat-model/) first; use this page when the focus remains "Reviews code for language-specific security vulnerabilities and practical hardening opportunities.".
- If the task is closer to "Forces clarification when requirements are too ambiguous for safe security-sensitive work.", check [Ask Questions If Underspecified Skill](/en/awesome-skills/ask-questions-if-underspecified/) first; use this page when the focus remains "Reviews code for language-specific security vulnerabilities and practical hardening opportunities.".
- If the task is closer to "Builds deep architectural context before security audit or investigation work.", check [Audit Context Building Skill](/en/awesome-skills/audit-context-building/) first; use this page when the focus remains "Reviews code for language-specific security vulnerabilities and practical hardening opportunities.".
- If the task is closer to "Uses static analysis tools such as CodeQL, Semgrep, and SARIF workflows.", check [Static Analysis Skill](/en/awesome-skills/static-analysis/) first; use this page when the focus remains "Reviews code for language-specific security vulnerabilities and practical hardening opportunities.".
