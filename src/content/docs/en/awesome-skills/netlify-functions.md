...
description: "Netlify Functions Skill: Builds serverless API endpoints and background tasks on Netlify."
---

# Netlify Functions Skill

Builds serverless API endpoints and background tasks on Netlify.

This page helps you decide whether to install or invoke `netlify/netlify-functions`, and how to keep the first use bounded.

## Quick facts

| Field | Value |
| --- | --- |
| Skill | `netlify/netlify-functions` |
| Category | [Deployment and Platform Ops](/en/awesome-skills/categories/deployment-platform/) |
| Directory listing | https://officialskills.sh/netlify/skills/netlify-functions |
| Primary source | https://officialskills.sh/netlify/skills/netlify-functions |
...
Start from the listing page: https://officialskills.sh/netlify/skills/netlify-functions

If the page provides an install command, copy the command from the listing instead of reconstructing it from the URL. If your agent client supports GitHub skill paths, use the primary source: https://officialskills.sh/netlify/skills/netlify-functions
...
- Whether `netlify/netlify-functions` is better than a one-off prompt for this task.
...
- If the task is closer to "Covers Cloudflare Workers, Pages, storage, AI, networking, security, and infrastructure configuration.", check [Cloudflare Skill](/en/awesome-skills/cloudflare/) first; use this page when the focus remains "Builds serverless API endpoints and background tasks on Netlify.".
- If the task is closer to "Reviews and authors Workers code against production best practices and `wrangler.jsonc` conventions.", check [Workers Best Practices Skill](/en/awesome-skills/workers-best-practices/) first; use this page when the focus remains "Builds serverless API endpoints and background tasks on Netlify.".
- If the task is closer to "Deploys and manages Workers, KV, R2, D1, Vectorize, Queues, and Workflows with Wrangler.", check [Wrangler Skill](/en/awesome-skills/wrangler/) first; use this page when the focus remains "Builds serverless API endpoints and background tasks on Netlify.".
- If the task is closer to "Deploys apps to Cloudflare using Workers, Pages, and related platform services.", check [Cloudflare Deploy Skill](/en/awesome-skills/cloudflare-deploy/) first; use this page when the focus remains "Builds serverless API endpoints and background tasks on Netlify.".
