---
title: "Swarms：CLI 与 YAML 工作流"
description: "Swarms 的CLI 与 YAML 工作流：包含来源链接、任务地图、取舍、坑点和更新基线的中文指南。"
---

# Swarms：CLI 与 YAML 工作流
...
- 核心问题：什么时候不用 Python 代码，改用 CLI/YAML？
- 差异化角度：把 CLI 作为“实验和运营入口”，不是库 API 的替代。
- 研究基线：2026-05-22/23
...
### 1. setup/config commands

这一节用来回答本页背后的实际问题：什么时候不用 Python 代码，改用 CLI/YAML？ 对 Swarms 来说，"setup/config commands" 不应该停留在概念解释，而要写清输入、期望输出、验证信号和能证明该行为的来源。
...
### 2. agent/chat/run-agents

这一节用来回答本页背后的实际问题：什么时候不用 Python 代码，改用 CLI/YAML？ 对 Swarms 来说，"agent/chat/run-agents" 不应该停留在概念解释，而要写清输入、期望输出、验证信号和能证明该行为的来源。
...
### 3. autoswarm/heavy-swarm/llm-council

这一节用来回答本页背后的实际问题：什么时候不用 Python 代码，改用 CLI/YAML？ 对 Swarms 来说，"autoswarm/heavy-swarm/llm-council" 不应该停留在概念解释，而要写清输入、期望输出、验证信号和能证明该行为的来源。
...
这一节用来回答本页背后的实际问题：什么时候不用 Python 代码，改用 CLI/YAML？ 对 Swarms 来说，"load markdown" 不应该停留在概念解释，而要写清输入、期望输出、验证信号和能证明该行为的来源。
...
这一节用来回答本页背后的实际问题：什么时候不用 Python 代码，改用 CLI/YAML？ 对 Swarms 来说，"YAML config" 不应该停留在概念解释，而要写清输入、期望输出、验证信号和能证明该行为的来源。
...
这一节用来回答本页背后的实际问题：什么时候不用 Python 代码，改用 CLI/YAML？ 对 Swarms 来说，"shell examples" 不应该停留在概念解释，而要写清输入、期望输出、验证信号和能证明该行为的来源。
...
这一节用来回答本页背后的实际问题：什么时候不用 Python 代码，改用 CLI/YAML？ 对 Swarms 来说，"团队复用" 不应该停留在概念解释，而要写清输入、期望输出、验证信号和能证明该行为的来源。
...
CLI commands、CLI tutorial、CLI configuration、examples/cli。
