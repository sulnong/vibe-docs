---
title: "Swarms：Structured Outputs"
description: "Swarms 的Structured Outputs：包含来源链接、任务地图、取舍、坑点和更新基线的中文指南。"
---

# Swarms：Structured Outputs
...
- 核心问题：如何让 Agent 输出可被程序消费的数据？
- 差异化角度：把 structured output 放进生产 pipeline，而不是只给格式示例。
- 研究基线：2026-05-22/23
...
这一节用来回答本页背后的实际问题：如何让 Agent 输出可被程序消费的数据？ 对 Swarms 来说，"output types" 不应该停留在概念解释，而要写清输入、期望输出、验证信号和能证明该行为的来源。
...
这一节用来回答本页背后的实际问题：如何让 Agent 输出可被程序消费的数据？ 对 Swarms 来说，"JSON schema" 不应该停留在概念解释，而要写清输入、期望输出、验证信号和能证明该行为的来源。
...
这一节用来回答本页背后的实际问题：如何让 Agent 输出可被程序消费的数据？ 对 Swarms 来说，"Pydantic" 不应该停留在概念解释，而要写清输入、期望输出、验证信号和能证明该行为的来源。
...
这一节用来回答本页背后的实际问题：如何让 Agent 输出可被程序消费的数据？ 对 Swarms 来说，"validation" 不应该停留在概念解释，而要写清输入、期望输出、验证信号和能证明该行为的来源。
...
### 5. workflow output_type

这一节用来回答本页背后的实际问题：如何让 Agent 输出可被程序消费的数据？ 对 Swarms 来说，"workflow output_type" 不应该停留在概念解释，而要写清输入、期望输出、验证信号和能证明该行为的来源。
...
这一节用来回答本页背后的实际问题：如何让 Agent 输出可被程序消费的数据？ 对 Swarms 来说，"错误处理" 不应该停留在概念解释，而要写清输入、期望输出、验证信号和能证明该行为的来源。
...
这一节用来回答本页背后的实际问题：如何让 Agent 输出可被程序消费的数据？ 对 Swarms 来说，"与 tools 返回值配合" 不应该停留在概念解释，而要写清输入、期望输出、验证信号和能证明该行为的来源。
...
structured-outputs、agent-configuration、api/agent。
