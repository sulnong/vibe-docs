...
description: "Insecure Defaults Skill：识别硬编码密钥、默认凭据和弱加密等不安全默认配置。"
---

# Insecure Defaults Skill

识别硬编码密钥、默认凭据和弱加密等不安全默认配置。

这个页面帮助你判断是否应该安装或调用 `trailofbits/insecure-defaults`，以及第一次使用时应该怎样控制范围。
...
| 目录页 | https://officialskills.sh/trailofbits/skills/insecure-defaults |
| 主要来源 | https://officialskills.sh/trailofbits/skills/insecure-defaults |
| 主要依赖 | 仓库访问权限、范围、语言/工具上下文和审查目标 |
| 第一次尝试 | 先定义资产、信任边界或 diff，再让 skill 检查风险。 |

## 安装与设置

从目录页开始：https://officialskills.sh/trailofbits/skills/insecure-defaults

如果页面提供安装命令，优先复制目录页上的命令，不要根据 URL 自行拼接。若你的 agent 客户端支持 GitHub skill 路径，可以使用主要来源：https://officialskills.sh/trailofbits/skills/insecure-defaults
...
- `trailofbits/insecure-defaults` 是否比临时提示词更适合当前任务。
...
- 如果你的任务更接近「按语言特性审查安全漏洞，并识别实际加固机会。」，优先看 [Security Best Practices Skill](/zh/awesome-skills/security-best-practices/)；如果重点仍是「识别硬编码密钥、默认凭据和弱加密等不安全默认配置。」，再使用当前页面。
- 如果你的任务更接近「通过识别资产、信任边界和攻击路径生成仓库级威胁模型。」，优先看 [Security Threat Model Skill](/zh/awesome-skills/security-threat-model/)；如果重点仍是「识别硬编码密钥、默认凭据和弱加密等不安全默认配置。」，再使用当前页面。
- 如果你的任务更接近「在需求过于模糊、无法安全处理时强制澄清。」，优先看 [Ask Questions If Underspecified Skill](/zh/awesome-skills/ask-questions-if-underspecified/)；如果重点仍是「识别硬编码密钥、默认凭据和弱加密等不安全默认配置。」，再使用当前页面。
- 如果你的任务更接近「在安全审计或调查前建立深入架构上下文。」，优先看 [Audit Context Building Skill](/zh/awesome-skills/audit-context-building/)；如果重点仍是「识别硬编码密钥、默认凭据和弱加密等不安全默认配置。」，再使用当前页面。
