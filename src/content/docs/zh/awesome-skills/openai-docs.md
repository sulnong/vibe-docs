...
description: "OpenAI Docs Skill：从 OpenAI 开发者文档中提供权威使用指导。"
---

# OpenAI Docs Skill

从 OpenAI 开发者文档中提供权威使用指导。

这个页面帮助你判断是否应该安装或调用 `openai/openai-docs`，以及第一次使用时应该怎样控制范围。
...
| 目录页 | https://officialskills.sh/openai/skills/openai-docs |
| 主要来源 | https://officialskills.sh/openai/skills/openai-docs |
| 主要依赖 | 目标文件、工作区、文档来源或已连接的知识系统 |
| 第一次尝试 | 先从一个文件或一条知识沉淀流程开始，并检查产物。 |

## 安装与设置

从目录页开始：https://officialskills.sh/openai/skills/openai-docs

如果页面提供安装命令，优先复制目录页上的命令，不要根据 URL 自行拼接。若你的 agent 客户端支持 GitHub skill 路径，可以使用主要来源：https://officialskills.sh/openai/skills/openai-docs
...
- `openai/openai-docs` 是否比临时提示词更适合当前任务。
...
- 如果你的任务更接近「创建、编辑和分析 Microsoft Word 文档。」，优先看 [DOCX Skill](/zh/awesome-skills/docx/)；如果重点仍是「从 OpenAI 开发者文档中提供权威使用指导。」，再使用当前页面。
- 如果你的任务更接近「提取文本、创建 PDF，并处理 PDF 表单和审查任务。」，优先看 [PDF Skill](/zh/awesome-skills/pdf/)；如果重点仍是「从 OpenAI 开发者文档中提供权威使用指导。」，再使用当前页面。
- 如果你的任务更接近「创建、编辑和分析 PowerPoint 演示文稿。」，优先看 [PPTX Skill](/zh/awesome-skills/pptx/)；如果重点仍是「从 OpenAI 开发者文档中提供权威使用指导。」，再使用当前页面。
- 如果你的任务更接近「创建、编辑和分析 Excel 电子表格。」，优先看 [XLSX Skill](/zh/awesome-skills/xlsx/)；如果重点仍是「从 OpenAI 开发者文档中提供权威使用指导。」，再使用当前页面。
