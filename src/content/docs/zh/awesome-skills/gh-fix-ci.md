...
description: "GitHub Fix CI Skill：通过检查日志调试并修复失败的 GitHub Actions 检查。"
---

# GitHub Fix CI Skill

通过检查日志调试并修复失败的 GitHub Actions 检查。

这个页面帮助你判断是否应该安装或调用 `openai/gh-fix-ci`，以及第一次使用时应该怎样控制范围。
...
| 目录页 | https://officialskills.sh/openai/skills/gh-fix-ci |
| 主要来源 | https://officialskills.sh/openai/skills/gh-fix-ci |
| 主要依赖 | 仓库、issue/PR 上下文、notebook 运行时或 skill 编写目标 |
| 第一次尝试 | 先作用于一个边界清楚的工作流，并保持产物可审查。 |

## 安装与设置

从目录页开始：https://officialskills.sh/openai/skills/gh-fix-ci

如果页面提供安装命令，优先复制目录页上的命令，不要根据 URL 自行拼接。若你的 agent 客户端支持 GitHub skill 路径，可以使用主要来源：https://officialskills.sh/openai/skills/gh-fix-ci
...
- `openai/gh-fix-ci` 是否比临时提示词更适合当前任务。
...
- 如果你的任务更接近「指导创建用于扩展 agent 专门工作流的 skills。」，优先看 [Skill Creator Skill](/zh/awesome-skills/skill-creator/)；如果重点仍是「通过检查日志调试并修复失败的 GitHub Actions 检查。」，再使用当前页面。
- 如果你的任务更接近「提供创建新 skill 的基础模板。」，优先看 [Template Skill](/zh/awesome-skills/template/)；如果重点仍是「通过检查日志调试并修复失败的 GitHub Actions 检查。」，再使用当前页面。
- 如果你的任务更接近「通过 CLI 处理 GitHub PR 上的 review 和 issue comments。」，优先看 [GitHub Address Comments Skill](/zh/awesome-skills/gh-address-comments/)；如果重点仍是「通过检查日志调试并修复失败的 GitHub Actions 检查。」，再使用当前页面。
- 如果你的任务更接近「为实验和教程创建清晰、可复现的 Jupyter notebooks。」，优先看 [Jupyter Notebook Skill](/zh/awesome-skills/jupyter-notebook/)；如果重点仍是「通过检查日志调试并修复失败的 GitHub Actions 检查。」，再使用当前页面。
