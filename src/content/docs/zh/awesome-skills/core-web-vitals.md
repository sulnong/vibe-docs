...
description: "Core Web Vitals Skill：诊断并改进 LCP、INP 和 CLS 问题。"
---

# Core Web Vitals Skill

诊断并改进 LCP、INP 和 CLS 问题。

这个页面帮助你判断是否应该安装或调用 `addyosmani/core-web-vitals`，以及第一次使用时应该怎样控制范围。
...
| 目录页 | https://officialskills.sh/addyosmani/skills/core-web-vitals |
| 主要来源 | https://officialskills.sh/addyosmani/skills/core-web-vitals |
| 主要依赖 | 根据 skill 不同，通常需要浏览器、CLI 或目标 URL |
| 第一次尝试 | 先在低风险页面或本地预览上运行，并产出一个具体发现。 |

## 安装与设置

从目录页开始：https://officialskills.sh/addyosmani/skills/core-web-vitals

如果页面提供安装命令，优先复制目录页上的命令，不要根据 URL 自行拼接。若你的 agent 客户端支持 GitHub skill 路径，可以使用主要来源：https://officialskills.sh/addyosmani/skills/core-web-vitals
...
- `addyosmani/core-web-vitals` 是否比临时提示词更适合当前任务。
...
- 如果你的任务更接近「通过终端里的 `playwright-cli` 驱动真实浏览器，完成导航、表单、snapshot、截图、信息提取和 UI 流程调试。」，优先看 [Playwright Skill](/zh/awesome-skills/playwright/)；如果重点仍是「诊断并改进 LCP、INP 和 CLS 问题。」，再使用当前页面。
- 如果你的任务更接近「使用 Playwright 相关工作流测试本地 Web 应用。」，优先看 [Webapp Testing Skill](/zh/awesome-skills/webapp-testing/)；如果重点仍是「诊断并改进 LCP、INP 和 CLS 问题。」，再使用当前页面。
- 如果你的任务更接近「通过交互式 JavaScript REPL 持续操作浏览器和 Electron 应用。」，优先看 [Playwright Interactive Skill](/zh/awesome-skills/playwright-interactive/)；如果重点仍是「诊断并改进 LCP、INP 和 CLS 问题。」，再使用当前页面。
- 如果你的任务更接近「围绕性能、可访问性、SEO 和最佳实践进行 Web 质量审查。」，优先看 [Web Quality Audit Skill](/zh/awesome-skills/web-quality-audit/)；如果重点仍是「诊断并改进 LCP、INP 和 CLS 问题。」，再使用当前页面。
