...
description: "Vercel Deploy Skill：把应用和网站部署到 Vercel 预览或生产环境。"
---

# Vercel Deploy Skill

把应用和网站部署到 Vercel 预览或生产环境。

这个页面帮助你判断是否应该安装或调用 `openai/vercel-deploy`，以及第一次使用时应该怎样控制范围。
...
| 目录页 | https://officialskills.sh/openai/skills/vercel-deploy |
| 主要来源 | https://officialskills.sh/openai/skills/vercel-deploy |
| 主要依赖 | 平台账号、CLI、项目配置和环境变量 |
| 第一次尝试 | 先用于预览、staging 服务或 dry-run 路径，再进入生产环境。 |

## 安装与设置

从目录页开始：https://officialskills.sh/openai/skills/vercel-deploy

如果页面提供安装命令，优先复制目录页上的命令，不要根据 URL 自行拼接。若你的 agent 客户端支持 GitHub skill 路径，可以使用主要来源：https://officialskills.sh/openai/skills/vercel-deploy
...
- `openai/vercel-deploy` 是否比临时提示词更适合当前任务。
...
- 如果你的任务更接近「覆盖 Cloudflare Workers、Pages、存储、AI、网络、安全和基础设施配置。」，优先看 [Cloudflare Skill](/zh/awesome-skills/cloudflare/)；如果重点仍是「把应用和网站部署到 Vercel 预览或生产环境。」，再使用当前页面。
- 如果你的任务更接近「按生产最佳实践和 `wrangler.jsonc` 约定审查、编写 Workers 代码。」，优先看 [Workers Best Practices Skill](/zh/awesome-skills/workers-best-practices/)；如果重点仍是「把应用和网站部署到 Vercel 预览或生产环境。」，再使用当前页面。
- 如果你的任务更接近「用 Wrangler 部署和管理 Workers、KV、R2、D1、Vectorize、Queues 和 Workflows。」，优先看 [Wrangler Skill](/zh/awesome-skills/wrangler/)；如果重点仍是「把应用和网站部署到 Vercel 预览或生产环境。」，再使用当前页面。
- 如果你的任务更接近「使用 Workers、Pages 和相关平台服务把应用部署到 Cloudflare。」，优先看 [Cloudflare Deploy Skill](/zh/awesome-skills/cloudflare-deploy/)；如果重点仍是「把应用和网站部署到 Vercel 预览或生产环境。」，再使用当前页面。
