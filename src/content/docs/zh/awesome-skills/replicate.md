...
description: "Replicate Skill：通过 Replicate 发现、比较和运行 AI 模型。"
---

# Replicate Skill

通过 Replicate 发现、比较和运行 AI 模型。

这个页面帮助你判断是否应该安装或调用 `replicate/replicate`，以及第一次使用时应该怎样控制范围。
...
| 目录页 | https://officialskills.sh/replicate/skills/replicate |
| 主要来源 | https://officialskills.sh/replicate/skills/replicate |
| 主要依赖 | 模型平台账号、API key、数据集或 SDK 上下文 |
| 第一次尝试 | 先用小型示例请求、数据集或模型操作，再进入生产流程。 |

## 安装与设置

从目录页开始：https://officialskills.sh/replicate/skills/replicate

如果页面提供安装命令，优先复制目录页上的命令，不要根据 URL 自行拼接。若你的 agent 客户端支持 GitHub skill 路径，可以使用主要来源：https://officialskills.sh/replicate/skills/replicate
...
- `replicate/replicate` 是否比临时提示词更适合当前任务。
...
- 如果你的任务更接近「提供使用 Gemini API 开发 Gemini 应用的最佳实践。」，优先看 [Gemini API Dev Skill](/zh/awesome-skills/gemini-api-dev/)；如果重点仍是「通过 Replicate 发现、比较和运行 AI 模型。」，再使用当前页面。
- 如果你的任务更接近「指导在 Google Cloud Vertex AI 上开发 Gemini 应用。」，优先看 [Vertex AI API Dev Skill](/zh/awesome-skills/vertex-ai-api-dev/)；如果重点仍是「通过 Replicate 发现、比较和运行 AI 模型。」，再使用当前页面。
- 如果你的任务更接近「使用 Hugging Face CLI 完成 Hub 操作。」，优先看 [Hugging Face CLI Skill](/zh/awesome-skills/hf-cli/)；如果重点仍是「通过 Replicate 发现、比较和运行 AI 模型。」，再使用当前页面。
- 如果你的任务更接近「使用 TRL 流程训练模型，包括 SFT、DPO、GRPO 和 GGUF 转换。」，优先看 [Hugging Face Model Trainer Skill](/zh/awesome-skills/hugging-face-model-trainer/)；如果重点仍是「通过 Replicate 发现、比较和运行 AI 模型。」，再使用当前页面。
