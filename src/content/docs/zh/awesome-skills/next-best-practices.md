...
description: "Next.js Best Practices Skill：应用推荐的 Next.js 模式和实现建议。"
---

# Next.js Best Practices Skill

应用推荐的 Next.js 模式和实现建议。

这个页面帮助你判断是否应该安装或调用 `vercel-labs/next-best-practices`，以及第一次使用时应该怎样控制范围。
...
| 目录页 | https://officialskills.sh/vercel-labs/skills/next-best-practices |
| 主要来源 | https://officialskills.sh/vercel-labs/skills/next-best-practices |
| 主要依赖 | 项目 UI 技术栈、设计上下文或框架版本 |
| 第一次尝试 | 先作用于一个页面、组件或设计决策，再考虑大范围改造。 |

## 安装与设置

从目录页开始：https://officialskills.sh/vercel-labs/skills/next-best-practices

如果页面提供安装命令，优先复制目录页上的命令，不要根据 URL 自行拼接。若你的 agent 客户端支持 GitHub skill 路径，可以使用主要来源：https://officialskills.sh/vercel-labs/skills/next-best-practices
...
- `vercel-labs/next-best-practices` 是否比临时提示词更适合当前任务。
...
- 如果你的任务更接近「用更强的布局、图片、层级和动效约束推动 composition-first 前端工作。」，优先看 [Frontend Skill](/zh/awesome-skills/frontend-skill/)；如果重点仍是「应用推荐的 Next.js 模式和实现建议。」，再使用当前页面。
- 如果你的任务更接近「指导前端设计和 UI/UX 开发工作。」，优先看 [Frontend Design Skill](/zh/awesome-skills/frontend-design/)；如果重点仍是「应用推荐的 Next.js 模式和实现建议。」，再使用当前页面。
- 如果你的任务更接近「应用 Vercel 风格的 Web 设计指南和标准。」，优先看 [Web Design Guidelines Skill](/zh/awesome-skills/web-design-guidelines/)；如果重点仍是「应用推荐的 Next.js 模式和实现建议。」，再使用当前页面。
- 如果你的任务更接近「按现代模式和实际约束审查、编写 React 代码。」，优先看 [React Best Practices Skill](/zh/awesome-skills/react-best-practices/)；如果重点仍是「应用推荐的 Next.js 模式和实现建议。」，再使用当前页面。
