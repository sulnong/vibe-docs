---
title: "Hermes Agent：安全模型与权限边界"
description: "Hermes Agent 的安全模型与权限边界：包含来源链接、任务地图、取舍、坑点和更新基线的中文指南。"
---

# Hermes Agent：安全模型与权限边界
...
- 核心问题：Hermes 的哪些机制是安全边界，哪些只是启发式保护？
- 差异化角度：突出官方安全政策里的关键判断：against adversarial LLM，只有 OS-level isolation 是真正边界。
- 研究基线：2026-05-22/23

## 编辑备注

- 官方安全政策非常明确：面对对抗性 LLM 时，真正承重的边界是操作系统级隔离。审批门、脱敏、扫描器和 allowlist 都是有用的启发式保护，不是容器边界。
- 只要要让 Hermes 接触不可信网页、入站邮件、公开群聊、第三方 skill、插件或 MCP server，就应该先读这一页。
...
### 1. official trust model

这一节用来回答本页背后的实际问题：Hermes 的哪些机制是安全边界，哪些只是启发式保护？ 对 Hermes Agent 来说，"official trust model" 不应该停留在概念解释，而要写清输入、期望输出、验证信号和能证明该行为的来源。
...
这一节用来回答本页背后的实际问题：Hermes 的哪些机制是安全边界，哪些只是启发式保护？ 对 Hermes Agent 来说，"OS-level isolation" 不应该停留在概念解释，而要写清输入、期望输出、验证信号和能证明该行为的来源。
...
### 3. terminal backend vs whole-process wrapping

这一节用来回答本页背后的实际问题：Hermes 的哪些机制是安全边界，哪些只是启发式保护？ 对 Hermes Agent 来说，"terminal backend vs whole-process wrapping" 不应该停留在概念解释，而要写清输入、期望输出、验证信号和能证明该行为的来源。
...
这一节用来回答本页背后的实际问题：Hermes 的哪些机制是安全边界，哪些只是启发式保护？ 对 Hermes Agent 来说，"dangerous command approval" 不应该停留在概念解释，而要写清输入、期望输出、验证信号和能证明该行为的来源。
...
这一节用来回答本页背后的实际问题：Hermes 的哪些机制是安全边界，哪些只是启发式保护？ 对 Hermes Agent 来说，"gateway auth" 不应该停留在概念解释，而要写清输入、期望输出、验证信号和能证明该行为的来源。
...
这一节用来回答本页背后的实际问题：Hermes 的哪些机制是安全边界，哪些只是启发式保护？ 对 Hermes Agent 来说，"credential filtering" 不应该停留在概念解释，而要写清输入、期望输出、验证信号和能证明该行为的来源。
...
### 7. skills/plugins trust

这一节用来回答本页背后的实际问题：Hermes 的哪些机制是安全边界，哪些只是启发式保护？ 对 Hermes Agent 来说，"skills/plugins trust" 不应该停留在概念解释，而要写清输入、期望输出、验证信号和能证明该行为的来源。
...
这一节用来回答本页背后的实际问题：Hermes 的哪些机制是安全边界，哪些只是启发式保护？ 对 Hermes Agent 来说，"MCP risk" 不应该停留在概念解释，而要写清输入、期望输出、验证信号和能证明该行为的来源。
...
这一节用来回答本页背后的实际问题：Hermes 的哪些机制是安全边界，哪些只是启发式保护？ 对 Hermes Agent 来说，"practical hardening checklist" 不应该停留在概念解释，而要写清输入、期望输出、验证信号和能证明该行为的来源。
...
security guide、SECURITY.md、Docker/Nix/OpenShell references、tools。
